...
void pid_init(void){
	...
	// Initialisation de l'unité de mesure inertielle (imu)
...
float pid_get_current_angle(void){
	...
	float real_accel_x = raw_accel_x * ACCELEROMETER_TO_G_MULTIPLIER;
	float real_accel_y = raw_accel_y * ACCELEROMETER_TO_G_MULTIPLIER;
	...
	float angular_velocity = GYROSCOPE_TO_ANGULAR_VELOCITY_MULTIPLIER*raw_gyro_z;
	...
	float theta_gyro_rad = theta_rad + theta_gyro_rad_increment;
	/***************************************************************************/
	
	
	/********************** COMBINAISON DES 2 ANGLES EN 1 **********************/
	// Ratio de l'angle final provenant du gyroscope et de l'accéléromètre
	const float GYROSCOPE_RATIO =  0.995;        			// Ratio du gyroscope désiré (faites des tests!)
	const float ACCELEROMETER_RATIO = 1. - GYROSCOPE_RATIO; // Les deux ratios ensembles doivent totaliser 1.
	...
	// Si on n'est pas au premier tour
		...
		theta_rad = theta_accel_rad * ACCELEROMETER_RATIO + theta_gyro_rad * GYROSCOPE_RATIO;
	}
...
float pid_tic(float desired_angle, float current_angle){
	...
	return (current_error * static_kp + delta_error * static_kd + cumulative_error * static_ki) * 100;	
}
